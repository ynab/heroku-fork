...
Apps.prototype = {
  ...
  createNewApp: function* (oldApp, newAppName, region, desiredOrg, space) {
    process.stdout.write(`Forking ${cli.color.cyan(oldApp.name)}... `);
    const orgOfOldApp = yield this.lookupOrgFromApp(oldApp);
    const info = {
      name: newAppName,
    ...
    try {
      if (orgOfOldApp) {
        ...
        info.organization = orgOfOldApp.name;
      } 
      ...
      if (!info.space){
        // If we were going into a private space, the desired org doesn't matter. It's implied by the space
        if (desiredOrg == "none"){
          // They can specify an empty organization, which means "don't put the new app in an org even if the original one was"
          info.organization = undefined;
        } else {
          info.organization = desiredOrg;
        }
      }
      ...
      if (info.organization) {
        message += ` in org ${cli.color.green(orgOfOldApp.name)}`;
      }
      if (info.space){
